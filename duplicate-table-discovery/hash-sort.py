...
def fpCheck(rowArray1, rowArray2, tid, rowMap2):
    global map1
    ...
    for i in range(0,len(rowArray1)):
        if i >= len(rowArray2):
            # fail
            return False
        if rowArray1[i] != rowArray2[i]:
            # fail, different values
            return False
        else:
            found_cm = False
            for j in map1[rowArray1[i]]:
                if j in column_mapping[tid]:
                    for h in column_mapping[tid][j]:
                        if h in rowMap2[rowArray1[i]]:
                            found_cm = True
                            break
                else:
                    found_cm = True
...
for result in cursor:
    ...
    if (tmp_tableid != -1 and tmp_tableid != result[0]) or (tmp_rowid != -1 and tmp_rowid != result[1]):
        ...
        for rowId in superKeyMapping[int(tmp_superkey, 2)]:
            ...
            if fpCheck(rows[0][rowId], row, tmp_tableid, map2): # Check for false positive
...
if tmp_tableid != -1:  # check that at least one row is found
    ...
    for rowId in superKeyMapping[int(tmp_superkey, 2)]:
        ...
        if fpCheck(rows[0][rowId], row, tmp_tableid, map2):
            dup.append((rowId, (tmp_tableid, tmp_rowid)))
            tableIds_length_to_load.add(tmp_tableid)
        else:
            counter_fp = counter_fp + 1
