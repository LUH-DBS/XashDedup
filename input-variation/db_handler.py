...
def getTableData(t1, t2, returnDict=False):
    ...
    cursor.execute(
        f'SELECT tableid, rowid, colid, tokenized, super_key FROM "mate_main_tokenized" WHERE tableid >= {t1} AND tableid <= {t2}')
...
def getTableDataSuperkeyOnly(t1, t2):
    ...
    cursor.execute(
        f'SELECT tableid, rowid, super_key, count(colid) as no_cols FROM "mate_main_tokenized" WHERE tableid >= {t1} AND tableid <= {t2} GROUP BY tableid, rowid, super_key')
    results = cursor.fetchall()
...
def getRowValues(t1, t1_rowid, t2, t2_rowid):
    ...
    cursor.execute(
        f'SELECT tableid, rowid, colid, tokenized FROM "mate_main_tokenized" WHERE (tableid = {t1} AND rowid = {t1_rowid}) OR (tableid = {t2} AND rowid = {t2_rowid})')
    results = cursor.fetchall()
...
def getRowValuesAll(tableIdRowId):
    ...
    cursor.execute(
        f'SELECT tableid, rowid, colid, tokenized FROM "mate_main_tokenized" WHERE ' + sql[:-4])
    results = cursor.fetchall()
